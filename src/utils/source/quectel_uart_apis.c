...
char rx_buff[1024];
char tx_buff[1024];
/* uart config para for all modules */
...
/* uart config para for all modules */
QT_UART_CONF_PARA *uart1_conf = &uart_conf;
...
void uart_init(void)
{
	...
	uart_cfg.baud_Rate			= uart1_conf->baudrate;
	...
	status = qapi_UART_Open(&uart1_conf->hdlr, uart1_conf->port_id, &uart_cfg);
	IOT_DEBUG("QT# qapi_UART_Open [%d] status %d", uart1_conf->port_id, status);

	status = qapi_UART_Power_On(uart1_conf->hdlr);
	IOT_DEBUG("QT# qapi_UART_Power_On [%d] status %d", uart1_conf->port_id, status);

	status = qapi_UART_Ioctl(uart1_conf->hdlr, QAPI_SET_FLOW_CTRL_E, &uart_fc_type);
	IOT_DEBUG("QT# qapi_UART_Ioctl [%d] status %d", uart1_conf->port_id, status);	
}
...
{
	qapi_Status_t status;
	status = qapi_UART_Transmit(uart1_conf->hdlr, buff, len, NULL);
	IOT_DEBUG("QT# qapi_UART_Transmit [%x] status %d", &uart1_conf->port_id, status);
}
...
{
	qapi_Status_t status;
	status = qapi_UART_Receive(uart1_conf->hdlr, uart1_conf->rx_buff, uart1_conf->rx_len, (void*)uart1_conf);
	IOT_DEBUG("QT# qapi_UART_Receive [%d] status %d", (qapi_UART_Port_Id_e)uart1_conf->port_id, status);
}
...
static void uart_rx_cb(uint32_t num_bytes, void *cb_data)
{
	uart1_conf = (QT_UART_CONF_PARA*)cb_data;
	...
	else if(num_bytes >= uart1_conf->rx_len)
	{
		num_bytes = uart1_conf->rx_len;
	}

#ifdef QT_UART_ECHO_ENABLE
	uart_print(uart1_conf->rx_buff, num_bytes);
#endif
	IOT_DEBUG("QT# uart_rx_cb data [%d][%s]", num_bytes, uart1_conf->rx_buff);
...
void qt_uart_dbg(const char* fmt, ...)
{
    ...
    qapi_UART_Transmit(uart1_conf->hdlr, log_buf, strlen(log_buf), NULL);
